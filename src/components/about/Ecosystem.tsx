...
const Ecosystem: React.FC<EcosystemProps> = ({
    title = "Our Ecosystem",
    subtitle = "Integrated Care Model",
    description,
    items = []
}) => {
    // Fallback if no items
    const displayItems = items.length > 0 ? items : [
         { title: "Clinics", description: "Primary care & diagnostics centers with modern facilities.", icon: "Building2", color: "blue" },
         { title: "Pharmacy", description: "Retail & Online access to affordable, authentic medicines.", icon: "Pill", color: "green" },
         { title: "Digital Health", description: "Teleconsultation, booking & EHR management platform.", icon: "Smartphone", color: "purple" },
         { title: "Diagnostics", description: "Central lab services and reliable home sample collection.", icon: "Activity", color: "rose" }
    ];
        ...
        <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                {displayItems.map((item, index) => {
                   ...
                   const itemColor = item.color || Object.keys(colors)[index % Object.keys(colors).length];
